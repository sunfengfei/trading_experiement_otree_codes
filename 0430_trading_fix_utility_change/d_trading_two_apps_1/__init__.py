...
def creating_session(subsession):
    for player in subsession.get_players():
        ...
        if subsession.round_number == 1:
            ...
            alpha = round(random.uniform(0.2, 0.8), 2)
            beta = round(random.uniform(0.2, 0.8), 2)
            endowment_a = random.randint(0, TOTAL_ENDOWMENT)
            endowment_b = TOTAL_ENDOWMENT - endowment_a

            # 存到 participant.vars
            participant.vars['utility_type'] = utility_type
            participant.vars['alpha'] = alpha
            participant.vars['beta'] = beta
            participant.vars['endowment_a'] = endowment_a
            participant.vars['endowment_b'] = endowment_b
            participant.vars['current_a'] = endowment_a
            participant.vars['current_b'] = endowment_b

            print(f"[Global init] Participant {participant.id_in_session} initialized:")
...
class Intro(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        participant.vars['student_id'] = player.student_id
        participant.vars['utility_type'] = player.utility_type
        participant.vars['alpha'] = player.alpha
        participant.vars['beta'] = player.beta
        participant.vars['endowment_a'] = player.endowment_a
        participant.vars['endowment_b'] = player.endowment_b
        participant.vars['current_a'] = player.current_a
        participant.vars['current_b'] = player.current_b
class ParameterDisplay(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        player.participant.vars['initial_utility'] = actual_utility
...
class Calculator(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        # STORE all variables persistently
        participant.vars['student_id'] = player.student_id
        participant.vars['utility_type'] = player.utility_type
        participant.vars['alpha'] = player.alpha
        participant.vars['beta'] = player.beta
        ...
        # Optional: also store calculated utility if already computed
        participant.vars['calculated_utility'] = player.calculate_utility()
