...
class Subsession(BaseSubsession):
    ...
    def group_by_partner_ids(self):
        ...
        for player in players:
            # 跳过已配对的玩家
            if player.id_in_subsession in matched:
                continue
                
            # 检查玩家是否选择了合作伙伴
            if player.partner_id:
                # 找到选择的伙伴
                partner = next((p for p in players 
                              if p.student_id == player.partner_id), None)
                
                # 检查是否互相选择
                if (partner and 
                    partner.partner_id == player.student_id and 
                    partner.id_in_subsession not in matched):
                    
                    # 添加到已配对集合
                    matched.add(player.id_in_subsession)
                    matched.add(partner.id_in_subsession)
                    new_groups.append([player, partner])
                else:
                    # 清除无效选择
                    player.partner_id = None

        # 处理未配对的玩家
        unmatched_players = [p for p in players if p.id_in_subsession not in matched]
        for player in unmatched_players:
            player.partner_id = None  # 清除未成功配对玩家的选择
            new_groups.append([player])  # 将未配对玩家单独分为一组
        
        # 更新分组矩阵
        if new_groups:
            matrix = [[p.id_in_subsession for p in group] for group in new_groups]
...
class MatchingWaitPage(WaitPage):
    title_text = "Please waiting for matching"
    body_text = "Waiting for other players to match"
    wait_for_all_groups = True
    
    def after_all_players_arrive(self):
        matched = self.subsession.group_by_partner_ids()
        if not matched:
            # If matching fails, send players back to Calculator page
            for p in self.subsession.get_players():
                p._index_in_pages = self.subsession._index_in_pages - 2
