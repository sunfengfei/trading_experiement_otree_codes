...
def creating_session(subsession):
    if subsession.round_number == 1:  # Only set initial values in first round
        ...
        # Copy values from previous round
        for player in subsession.get_players():
            previous_player = player.in_round(subsession.round_number - 1)
            player.utility_type = previous_player.utility_type
            player.alpha = previous_player.alpha
            player.beta = previous_player.beta
            player.endowment_a = previous_player.current_a  # Use final holdings from previous round
            player.endowment_b = previous_player.current_b
            player.current_a = previous_player.current_a
            player.current_b = previous_player.current_b
...
class Calculator(Page):
    ...
    def is_displayed(player):
        # Skip this page in round 1 if student_id hasn't been entered
        if player.round_number == 1 and player.field_maybe_none('student_id') is None:
            return False
        return True
...
class MatchingWaitPage(WaitPage):
    ...
    @staticmethod
    def is_displayed(player):
        # Only display wait page if both student_id and partner_id are set
        return (player.field_maybe_none('student_id') is not None and 
                player.field_maybe_none('partner_id') is not None)
    ...
    def group_by_arrival_time_method(subsession, waiting_players):
        ...
        for p1 in waiting_players:
            if not p1.partner_id:  # Skip players without partner_id
                print(f"Skipping player {p1.student_id} - no partner_id")
                continue
                
            # Find matching partner in waiting players
            p2 = next(
                (p for p in waiting_players 
                 if p != p1 and  # Not same player
                 p.student_id == p1.partner_id and  # p2's ID matches p1's chosen partner
                 p.partner_id == p1.student_id),  # Ensure reciprocal matching
                None
            )
...
class Results(Page):
    def vars_for_template(self):
        return {
            ...
            'round_number': self.round_number,
            'total_rounds': C.NUM_ROUNDS,
            'is_final_round': self.round_number == C.NUM_ROUNDS
        }

page_sequence = [
    Intro,
    # ParameterDisplay,  # Moved after Intro
    Calculator,
    MatchingWaitPage,  # Moved after Calculator
    Trading,
    TradeWaitPage,
    Results
]